...
import { SessionStore, SessionData, runStalenessCheck } from '@unit01/core/session/index.js';
...
async function main() {
  ...
  const LOOP_TRIGGER_COUNT = isPro() ? 2 : 3; // Block duplicate tool calls on 2nd consecutive identical call (not 3rd)
...
${activeChanges}`;
      ...
      if (command === '/help') {
        ...
        const helpItems = [
          ...
          ...(isPro() ? [{ cmd: '/diff', desc: 'show a colored diff of all session changes' }] : []),
      ...
      if (command === '/mcp') {
        const { McpClientManager } = await import('@unit01/core/mcp/client.js');
        const { loadMcpConfig, saveMcpServer, removeMcpServer } = await import('@unit01/core/mcp/config.js');
